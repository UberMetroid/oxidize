//! All reactive game state signals bundled into one struct.

use leptos::*;
use oxidize_engine::PlayerState;
use crate::types::UpgradeEffect;

/// Bundles every game signal for clean propagation to child components.
...
/// Creates all signals with sensible defaults.
pub fn create_game_signals(initial_state: PlayerState) -> GameSignals {
    GameSignals {
        ...
        planet_angles: create_signal(vec![0.0; 8]),
        moon_angles: create_signal(vec![0.0; 14]),
        ...
        fly_from_x: create_signal(0.0),
        fly_from_y: create_signal(0.0),
        fly_to_x: create_signal(0.0),
        fly_to_y: create_signal(0.0),
        fly_x: create_signal(0.0),
        fly_y: create_signal(0.0),
        fly_vx: create_signal(0.0),
        fly_vy: create_signal(0.0),
        planet_offset: create_signal(0.0),
